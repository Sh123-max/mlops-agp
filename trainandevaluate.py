...
try:
    ...
    class MetricsHistory:
        ...
        def get_previous_best(self, *args, **kwargs): return None
...
# Optional scaler and unscaled df, used in metadata baseline
scaler = None
try:
...
models = {
    ...
    "XGBoost": XGBClassifier(
        ...
        eval_metric="logloss",
        n_jobs=max(1, NUM_WORKER_THREADS),
        verbosity=1,
        random_state=42,
    ),
...
def train_and_log(name, model):
    ...
    try:
        with mlflow.start_run(run_name=run_name) as run:
            ...
            try:
                mlflow.sklearn.log_model(model, artifact_path="model")  # [web:21]
                try:
            ...
            # Prometheus gauges for this model
            ...
            print(
                f"[{name}] done: weighted_score={weighted:.4f}, "
                f"retrain_time={retrain_time:.2f}s, fn_rate={fn_rate:.4f}"
            )
            return {
                "name": name,
                "run_id": run_id,
...
with ThreadPoolExecutor(max_workers=NUM_WORKER_THREADS) as ex:
    ...
    for fut in as_completed(futures):
        ...
        try:
            ...
            if "error" not in res:
                print(
                    f"[MAIN] Collected result for {name} "
                    f"weighted_score={res.get('weighted_score')}"
                )
        except Exception as e:
...
# Build objective vectors for Pareto front
objs = []
...
if pareto_models:
    ...
    best = {
        "score": float(chosen.get("weighted_score", -1)),
        "name": chosen["name"],
...
# Deployment decision vs previous best
...
if best.get("run_id") and best.get("name") and should_deploy:
    ...
    ok = False
    print(
        f"[MAIN] Waiting up to {max_wait_sec}s for artifacts for run {run_id} "
    ...
    if not ok:
        print(
            f"[MAIN] Warning: artifacts for run {run_id} not found after "
            f"{max_wait_sec}s. Attempting registration anyway."
        )

    try:
        print(f"[MAIN] Registering model runs:/{run_id}/model -> {registry_name}")
        registered = mlflow.register_model(f"runs:/{run_id}/model", registry_name)  # [web:20][web:23]
        best["registry"] = {
            "name": registry_name,
            "version": getattr(registered, "version", None),
        }
        print("[MAIN] Registered model:", best["registry"])
        metrics_history.add_deployment(best)
    except Exception as e:
...
try:
    if X_train_unscaled_df is not None:
        ...
        feature_distributions = {
            feat_names[i]: X_train_unscaled[:, i].tolist() for i in range(n_feats)
        }
        feature_means = {
            feat_names[i]: float(np.nanmean(X_train_unscaled[:, i])) for i in range(n_feats)
        }
        feature_stds = {
            feat_names[i]: float(np.nanstd(X_train_unscaled[:, i])) for i in range(n_feats)
        }
...
try:
    ...
    if chosen_res:
        ...
        try:
            ...
            print(
                f"[MONITOR] export_retrain_time called with {retrain_t}s "
                f"for job={PROJECT_NAME}_retrain"
            )
        except Exception as e:
...
if PUSHGATEWAY_URL:
    try:
        ...
        print("[PROM] Prometheus push_to_gateway failed, falling back to raw text:", e)
        # Fallback to legacy text payload so existing dashboards still work
        try:
            if best.get("name") and should_deploy:
                payload_lines = []
                payload_lines.append(
                    f'model_weighted_score{{project="{PROJECT_NAME}",model="{best["name"]}"}} {best["score"]}'
                )
                chosen_res = None
                for r in results:
                    if r.get("name") == best.get("name"):
                ...
                if chosen_res:
                    payload_lines.append(
                        f'retrain_time_seconds{{project="{PROJECT_NAME}",model="{best["name"]}"}} '
                        f'{chosen_res.get("retrain_time_seconds", -1)}'
                    )
                    payload_lines.append(
                        f'false_negative_rate{{project="{PROJECT_NAME}",model="{best["name"]}"}} '
                        f'{chosen_res.get("false_negative_rate", -1)}'
                    )
                    payload_lines.append(
                        f'inference_latency_seconds{{project="{PROJECT_NAME}",model="{best["name"]}"}} '
                        f'{chosen_res.get("latency", -1)}'
                    )
                payload = "\n".join(payload_lines) + "\n"
                ...
                print("Pushgateway status (fallback):", resp.status_code)
        except Exception as e2:
            print("Push to pushgateway (fallback) failed:", e2)
