// Jenkinsfile (Declarative) - agent any + venv-based, uses bash -lc for bashisms/pipefail
pipeline {
    agent any
    ...
    environment {
        ...
        // Remove or override MLFLOW_TRACKING_URI if you don't use MLflow
    ...
    stages {
        ...
        stage('Prepare Python env & deps (venv)') {
            steps {
                // Use bash -lc so `set -o pipefail` and other Bash features are available on nodes where /bin/sh is dash
                sh '''bash -lc "
set -euo pipefail
...
# create venv (idempotent-ish for per-build)
python3 -m venv \"${VENV_DIR}\"
. \"${VENV_DIR}/bin/activate\"

# upgrade packaging tools to maximize chance of getting wheels
python -m pip install --upgrade pip setuptools wheel

# install dependencies preferring binary wheels to avoid heavy compilations (scipy)
if [ -f \"${REQUIREMENTS}\" ]; then
  ...
  pip install --no-cache-dir --prefer-binary pandas numpy scikit-learn joblib prometheus_client prometheus-flask-exporter || INSTALL_RC=\$?
fi

# fallback: if pip failed and INSTALL_BUILD_DEPS=true, install minimal system build deps and retry
...
set -euo pipefail
. \"${VENV_DIR}/bin/activate\"
# run deploy; allow deploy.py to exit non-zero without killing the pipeline here if you prefer - keep it strict for now
python deploy.py --project \"${PROJECT_NAME}\" --stage Staging || true

# ensure a deployment_time.txt exists (deploy.py may create it)
